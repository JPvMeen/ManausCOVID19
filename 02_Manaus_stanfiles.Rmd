...
```{r}
set_cmdstan_path("C:/Users/JanPaul/Documents/.cmdstan/cmdstan-2.33.1")
```

```{r}
dir_job_stan = "/job/stan"
```

#### wave 2 updated added epsilon
```{stan, engine = "cmdstan", output.var="manaus_2w_updated_s1re_strainfreq_RK_NB_survey"}
functions {
...
```{r}
write_stan_file(manaus_2w_updated_s1re_strainfreq_RK_NB_survey$code(), 
                             dir = dir_job_stan,
                             basename = "manaus_2w_updated_s1re_strainfreq_RK_NB_survey"
                             ) 
```


### wave 2 SIRS
```{stan, engine = "cmdstan", output.var="manaus_2w_updated_s1re_strainfreq_RK_NB_survey_SIRS"}
functions {
  vector one_strain_corona_model(
      real t, vector y, 
      real beta1, real gamma, real log10_ifr1_intercept, real log10_ifr1_slope,
...
```{r}
write_stan_file(manaus_2w_updated_s1re_strainfreq_RK_NB_survey_SIRS$code(), 
                             dir = dir_job_stan,
                             basename = "manaus_2w_updated_s1re_strainfreq_RK_NB_survey_SIRS"
                             ) 
```